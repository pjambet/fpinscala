...
trait Stream[+A] {
  ...
  final def toListRecursive: List[A] = this match {
  ...
  def foldRight[B](z: => B)(f: (A, => B) => B): B = // The arrow `=>` in front of the argument type `B` means that the function `f` takes its second argument by name and may choose not to evaluate it.
    this match {
      //        println(s"head: ${h()}")//, tail: ${t()}")
      case c @ Cons(h, t) =>
//        println(s"foldRight: h/t - ${h()}/${t()}")
...
//  def foldRight[B](z: => B)(f: (A, => B) => B): B = ???
  ...
  def exists(p: A => Boolean): Boolean =
    foldRight(false)((a: A, b) => p(a) || b) // Here `b` is the unevaluated recursive step that folds the tail of the stream. If `p(a)` returns `true`, `b` will never be evaluated and the computation terminates early.
  ...
  // Stream(1,2,3).foldRight(empty[Int])(fn)
  // cons(1, Stream(2,3).foldRight(fn))
  // fn(1, cons(2, Stream(3, empty)).foldRight(fn))
  // fn(2, empty)
  //
...
//      println(s"Filtering $h -> ${f(h)} - $this")
   ...
   */
  // TODO: Write something that traces the execution
...
object Stream {
  ...
  val trues: Stream[Boolean] = Stream.cons(true, trues)
