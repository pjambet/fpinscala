...
object RNG {
  ...
  def nonNegativeInt_s: State[RNG, Int] = {
    State(rng => {
      val (i, r) = rng.nextInt
      (if (i < 0) -(i + 1) else i, r)
    })
  ...
  def double_s: State[RNG, Double] =
    nonNegativeInt_s.map { i =>
  ...
  def boolean_s: State[RNG, Boolean] =
    State((r: RNG) => {
      val (i, r2) = r.nextInt
      (i % 2 == 0, r2)
    })
...
//  val _: Function1[RNG, (Int, RNG)] = ???
  ...
  def __map[A, B](s: RNG => (A, RNG))(f: A => B): RNG => (B, RNG) =
    flatMap(s)(a => unit(f(a)))

//  def ___map[A, B, S](s: S => (A, S))(f: A => B): S => (B, S) =
//    flatMap(s)(a => unit(f(a)))
